...
Function Run-FirstSetup {
	...
	Process {
		Try {
			...
			$Proceed = Read-Host "`t[Input]";
	
			If (($Proceed -Like "n") -Or ($Proceed -Like "no")){
			...
			Write-Host "`t(1/3) hMailServer can only be modified using the administrator password. Please enter this in the popup window. This will be encrypted.";
			...
			Write-Host "`t(2/3) A folder is required to store hMailServer's SSL / TLS certificates. Please enter the path below:";
			...
			Write-Host "`t(3/3) OpenSSL's EXE file is required to create certain certificate files. Please enter the path below:";
			
			$OpenSSL_PathAndName_Validity = $False;
			While ($OpenSSL_PathAndName_Validity -Eq $False){
				$OpenSSL_PathAndName = Read-Host "`t`t[Input]";
				
				# Remove quotation marks, if any
				$OpenSSL_PathAndName = $OpenSSL_PathAndName -Replace '"','';
				$OpenSSL_PathAndName = $OpenSSL_PathAndName -Replace "'","";
				
				If (($OpenSSL_PathAndName) -And (Test-Path -Path $OpenSSL_PathAndName -PathType Leaf)) {
					Write-Host -ForegroundColor Green "`t`t[Success]: File path validated.";
					
					$OpenSSL_PathAndName_Validity = $True;
				} Else {
					Write-Host -ForegroundColor Red "`t`t[ERROR]: File path is invalid. Please ensure that the path is valid and that this script has access to the folder.";
...
Function Update-hMailServerCertificate {
	...
	Process {
		Try {			
			...
			If (($hMailServer_AdminPassword_Secure) -And ($Script_ConfigFile_Content) -And ($hMailServer_SSLCert_Path) -And ($OpenSSL_PathAndName)) {
			...
			Write-Host "`t`tObtaining certificate...";
			...
			If ($Windows_SSLCert){
				...
				If (-Not $Thumbprint){
					Write-Host -ForegroundColor Green "`t`t`t[Success]: Thumbprint not specified so obtained newest certificate from store Local Computer | Personal | Certificates.";
				} Else {
					Write-Host -ForegroundColor Green "`t`t`t[Success]: Thumbprint specified so obtained matching certificate.";
				}
			...
			& $OpenSSL_PathAndName pkcs12 -in $Windows_SSLCert_File_PFX -passin pass:$Windows_SSLCert_Password_Plaintext -nocerts -nodes -out $Windows_SSLCert_File_KEY_Private;
			...
			& $OpenSSL_PathAndName pkcs12 -in $Windows_SSLCert_File_PFX -passin pass:$Windows_SSLCert_Password_Plaintext -clcerts -nokeys -out $Windows_SSLCert_File_CRT_Public;
...
If ($RunAsAdministrator -Eq $False) {
	...
	If (($Elevate -Like "y") -Or ($Elevate -Like "yes")){
		...
		$Arguments = ' -NoExit -File "' + $PSCommandPath + '"' + $AllParameters_String;
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUv4aFU8VC3n7hvuQXDctUWvP+
# Miigghn2MIIEhDCCA2ygAwIBAgIQQhrylAmEGR9SCkvGJCanSzANBgkqhkiG9w0B
...
# KoZIhvcNAQkEMRYEFNV27uv2TsBQet23UGHB1j0sqkWeMA0GCSqGSIb3DQEBAQUA
# BIIBAJU8nNo8VYodiOGX/OnDjc0bATzbYz33uPfSDP8jTUAos1poADv2gMzS2bth
# YTNC4GqVYlNTrZAUlQ1LT78WaAN18ft9xydagsRTLO4qq9Tp61MABK8yBBcjz2q2
# urfkblGyFshxIKwe28q2zHjNueIoFl3Y06sJVZYkE0xuVdTuTwjzYkl8hAIWc7Zc
# Qy9KFvUUN5Qw4Sc05zU8EIIMAEM9kfO3Y8vWtaYci06XMraKaYiEA8nPxZcE57IN
# Sat4W2W8LdAYLXAM5Nt+s1SGMPQN43qsqnIhPc346pP0xX35FCIaQgCeY2hCPGIO
# DmOyWWQx7I//g7/5Bm0esredBduhggIoMIICJAYJKoZIhvcNAQkGMYICFTCCAhEC
...
# SIb3DQEJBTEPFw0xOTA4MjIxMjI0MjhaMCMGCSqGSIb3DQEJBDEWBBRGcLaw/wP5
# lfVbE8cDzpdxBLecEjANBgkqhkiG9w0BAQEFAASCAQBQsbHaJwmNQrsc9dy5umeC
# xYKU7ssf+Pv0V3XXlFh4mE5Q0VS0n7qrc2egl6PJcV0/8LS8ZpawAwWawfnbxy6H
# kGqSCx6Djmkbqr3UWiT5iMXgVtavcrh5fypQzsFBiB/+h/zOqu+2zzk9lPOxzoOF
# qwAyczFY4pMaeW3mLzOydDAUea4fFGZyh7BmsIze0WhmzuUwZ9MKuhBi2jApRNHl
# Xz9HOar5JlIw97lhvV3h79nhBYsbxr4JeJPFTibisPSNmli9YJxyZZ6Ow0oPqZsM
# pyAOqspoOrMgclFu1QIv+v8+Gux00TOE9sC2bHzf/tBAQZ1kqfaPopAbEj90VjKQ
